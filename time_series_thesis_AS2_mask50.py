...
if __name__ == "__main__":
    ...
    tra_year = "2019"
    cali = ""
    model_type_list = ["Naive_LSTM_cum_obs_scratch",
                       "MC_base_prior13_01_convergence_obs_scratch",
                       ]
    model_py_list = ["Naive_LSTM_cum",
                       "MC_base_prior13_01",
                       ]
    ...
    #     R2, RMSE = utils.show_fit(x,y,title,14,max(max(x),max(y)),1,1,fig_type = "normal",markersize = 1, unit = unit,save_dir="figure/paper/MC")
    ...
    nrows = 6
    ncols = 1
    fig, axs = plt.subplots(dpi = 300,nrows=nrows, ncols=ncols, figsize=(3, 9))
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            axs_ij.set_ylabel("%s(%s)"%(obs_tpt[i*ncols + j],units[tpt_loc]))  # Add y-axis label
            ...
            axs_ij.text(0.03, 0.95, "(%s2)"%(chr(97 + tpt_loc-1)), transform=axs_ij.transAxes,
