...
if __name__ == "__main__":
    ...
    for seed in range(0,1):
        ...
        for model_type,model_py in zip(model_type_list,model_py_list):
            ...
            nrows = 2
            ncols = 3
            fig, axs = plt.subplots(dpi = 300,nrows=nrows, ncols=ncols, figsize=(7, 3))
            ...
            for i in range(nrows):
                for j in range(ncols):
                    ...
                    axs_ij.plot(day[(res>=0)*(day>=0)],res[(res>=0)*(day>=0)],c='blue',linewidth=1,label="ORYZA2000")
                    axs_ij.plot(day[(res>=0)*(day>=0)],fit[(res>=0)*(day>=0)],c="orange",linewidth=0.75)
                    ...
                    axs_ij.set_ylabel("%s"%(units[tpt_loc]))  # Add y-axis label
                    ...
                    axs_ij.text(0.72, 0.15, "(%s%d)"%(chr(97 + tpt_loc-1),fig_num), transform=axs_ij.transAxes,
                    ...
                    # plt.legend()
                    # title = "%s"%(obs_tpt[i*ncols + j])
                    # axs_ij.text(0.42, 0.95, title, transform=axs_ij.transAxes,
                    # fontsize=12, fontweight='bold', va='top')
