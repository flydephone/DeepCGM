...
def base_dataset_loader():
    ...
    mean_std = pickle_load('base_dataset/mean_std.pickle')
    par_col_name = pickle_load('base_dataset/par_col_name.pickle')
...
def unscalling(mode,data,max_array,min_array,mean,std):
    if mode == "nor":
        b = max_array - min_array
        data_unsca = data * b + min_array
    elif mode== "sta":
        data_unsca = data*std+mean
    else:
        print("mode: nor or sta")
    return data_unsca
