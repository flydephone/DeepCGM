    ...
    for seed in range(0,1):
        ...
        for model_type,model_py in zip(model_type_list,model_py_list):
            ...
            for trained_model_name in trained_model_names:
                epoch = int(trained_model_name.split("_")[2])
                # dvs super parameter  
                pre_list = []
                ...
                # 2018-2019: 4, -1
                # 2019-2018: -2, -3
                obs_tpt = ["PAI","WAGT","YIELD"]
                titles = ['植被面积指数','地上生物量',"产量"]
                ...
                for i in range(nrows):
                    for j in range(ncols):
                        ...
                        axs_ij.text(0.4, 0.95, "Epoch=%03d"%epoch, transform=axs_ij.transAxes,
                        fontsize=14, fontweight='bold', va='top')
                        ...
                        # plt.title(title)
                        utils.find_or_make("figure/series_%s"%model_type)
                        
                # plt.legend()
                plt.savefig('figure/series_%s/%s_%02d_%03d.png'%(model_type,tra_year,seed,epoch), bbox_inches='tight')
                # plt.show()
                # plt.close()
