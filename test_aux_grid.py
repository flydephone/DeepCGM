...
if __name__ == "__main__":
    ...
    for model in model_list[:1]:
        ...
        for i,(title,unit) in enumerate(zip(obs_name,units)):
            ...
            axs[i].set_ylabel("%s(%s)"%(obs_name[i],units[i]))
            ...
            RMSE = (RMSE / len(x)) ** 0.5
            axs[i].text(x=0.25 * uplim, y=0.025 * uplim, s='RMSE=' + str(RMSE)[:5],fontsize=fontsize,c="black")
 
            axs[i].axis('square')
            # plt.xlim([0, uplim])
            # plt.ylim([0, uplim])
        # fig.text(-0.4, 0.5,"Prediction", va='center', rotation='vertical',fontsize = 20)
        save_dir = "figure/paper/MC-DeepOryza_01_convergence_2019-2018"
