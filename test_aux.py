...
if __name__ == "__main__":
    ...
    for model in model_list[:1]:
        ...
        for i,(title,unit) in enumerate(zip(obs_name,units)):
            ...
            #     r = r[(x-1.2)*(x-1.8)>=0]
            #     x = x[(x-1.2)*(x-1.8)>=0]
            # save_dir = "figure/%s/%s"%(tes_year,"oryza2000")
            if title=="WRR14": title = "YIELD"
            R2, RMSE = utils.show_fit(x,y,title,14,max(max(x),max(y)),1,1,fig_type = "normal",markersize = 1, unit = unit,save_dir=None)
            # R2, RMSE = utils.show_fit(x,f,title,14,max(max(x),max(f)),1,1,fig_type = "normal",markersize = 1, unit = unit,save_dir=None)
            # R2, RMSE = utils.show_fit(x,z,title,24,max(max(x),max(z)),1,1,fig_type = "normal",markersize = 1, unit = unit,save_dir="figure/paper/ORYZA2000_2018-2018")
            r2_obs.append(R2)
            rmse_obs.append(RMSE)
        # obs_name = ['WLV']
        obs_name = ['DVS','PAI','WLV','WST','WSO','WAGT',"WRR14"]
        # sample_loc = 9
        for sample_loc in range(0,0):
            # np_wea_fer_sample = np_wea_fer_dataset[sample_loc]
            # plt.plot(np_wea_fer_sample[:,1],np_wea_fer_sample[:,3])
            # plt.show()
            ...
            for obs_tpt in ['DVS','PAI','WLV','WST','WSO','WAGT',"WRR14"]:
                ...
                fit = np_fit_dataset[sample_loc,:,tpt_loc]
                plt.plot(day[(res>=0)*(day>=0)],pre[(res>=0)*(day>=0)],c='red',label="pre")
                ...
                # plt.savefig('figure/%s_%s.png'%(obs_tpt,sample_loc), bbox_inches='tight')
                plt.show()
                plt.close()
