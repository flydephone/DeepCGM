...
if __name__ == "__main__":
    ...
    for seed in range(0,1):
        ...
        for model_type,model_py in zip(model_type_list,model_py_list):
            ...
            nrows = 6
            ncols = 1
            fig, axs = plt.subplots(dpi = 300,nrows=nrows+1, ncols=ncols, figsize=(1.5, 9))
            ...
            day = np_wea_fer_dataset[sample_loc,:,0]
            fer = np_wea_fer_dataset[sample_loc,:,-2]
            axs[6].bar(day[(res>=0)*(day>=0)],fer[(res>=0)*(day>=0)],color="blue",width = 4)
            axs[6].set_ylim(top=100)
            axs[6].set_ylabel("Nitrogen(kg/ha)")  # Add y-axis label
            axs[6].set_yticklabels(axs[6].get_yticks(), rotation=90)
            axs[6].yaxis.set_major_formatter(formatter)
            axs[6].text(0.03, 0.95, "(g%d)"%fig_num, transform=axs[6].transAxes,fontsize=14, fontweight='bold', va='top')
            axs[6].set_xlabel("Day of year")
            # plt.legend()
